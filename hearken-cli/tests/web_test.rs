...
async fn test_web_server_endpoints() {
    ...
    assert!(
        body.contains("Hearken Dashboard"),
        "Dashboard HTML not returned"
    );
    ...
    assert!(
        json["pattern_count"].as_i64().unwrap() > 0,
        "Expected patterns in summary"
    );
    assert!(json["total_occurrences"].as_i64().unwrap() > 0);
    assert!(!json["file_groups"].as_array().unwrap().is_empty());
    ...
    let p = json["patterns"]
        ...
        .unwrap()
        .iter()
        .find(|p| p["id"].as_i64().unwrap() == pattern_id)
        .unwrap();
